...
- Location: tests\SaucedemoLogin_Products.spec.ts:35:5
...
```ts
  ...
  3  | 
  4  | /**
  5  |  * @author: Ketan Tiwari
  6  |  * @date: 2026-04-11
  7  |  * @description: This is the base page class which will be extended by all the page classes. It contains the common methods which can be used by all the page classes.
  8  |  * 
  9  |  */
  10 | class BasePage {
  11 | 
  ...
  14 |   /**
  15 |    * 
  16 |    * @param {import("@playwright/test").Page} page - This is the page object which is passed as a parameter to the constructor. 
  17 |    * It is used to perform the actions on the page. 
  18 |    * @returns {void} This constructor will initialize the page object which is passed as a parameter to the constructor. 
  19 |    */
  ...
  25 |    * 
  26 |    * @param {String} url 
  27 |    * @returns {Promise<void>} This method will open the url passed as parameter and return the response of the request. 
  28 |    */
  29 |   async open_url(url:string): Promise<void> {
  ...
  37 |    * @returns {Promise<*>} This method will wait for the selector to be visible and then fill the text in the input field.
  38 |    */
  39 |   async waitAndFill(selector :string, text:string): Promise<void>  {
  40 |      await this.page.locator(selector).fill(text);
  41 |   }
  42 | 
  43 |   
  44 |   async tobeVisible(selector :string) {
  45 |      await expect(this.page.locator(selector)).toBeVisible();
  46 |   }
  47 | 
  48 |   async tohaveText(selector :string, text :string) {
> 49 |     await expect.soft(this.page.locator(selector)).toHaveText(text);
     |                                                    ^ Error: expect(locator).toHaveText(expected) failed
  50 |   }
  51 | 
  52 |   getbytestid(testid: string) {
  53 |     return this.page.locator(`[data-testid="${testid}"] , [data-test="${testid}"]`);
  54 |   }
  55 | 
  56 | 
  57 |   async openlinksinnewtab(selector :string) : Promise<Page> {
  58 |     const [newPage] = await Promise.all([
  59 |       this.page.context().waitForEvent('page'),
  60 |       this.getbytestid(selector).click()
  61 |     ]);
  62 | 
  ...
  68 |   }
  69 | 
  70 |   async waitandclick(selector :string) {
  71 |     await this.page.locator(selector).click();
  72 | 
  73 | }
  74 | 
  75 |   async clickonaddtocart(selector_class :string, selector_id :string) {
  76 |     await this.page.locator(selector_class+selector_id).click();
  77 |   }
  78 | 
  79 | 
  80 |   async clickonproductcard(selector :string, productname :string) {
  81 | 
  82 |       await this.page.locator(selector).filter({ hasText: productname }).click();
  83 |   }
  84 | 
  85 | }
  86 | 
  87 | export default BasePage;
```